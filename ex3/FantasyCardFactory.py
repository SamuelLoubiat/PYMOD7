...
class FantasyCardFactory(CardFactory):
    ...
    def __init__(self) -> None:
        self._creatures = [
            ...
            {"name": "Goblin Warrior", "cost": 2, "rarity": "Common",
             "attack": 2, "health": 1},
            {"name": "Ice Wizard", "cost": 4, "rarity": "Rare", "attack": 3,
             "health": 4},
    ...
    def create_creature(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power is None:
            creature = random.choice(self._creatures)
            return CreatureCard(creature["name"], creature["cost"],
                                creature["rarity"], creature["attack"],
                                creature["health"])
        ...
        if isinstance(name_or_power, int):
            for creature in self._creatures:
                if creature["cost"] == name_or_power:
                    return CreatureCard(creature["name"], creature["cost"],
                                        creature["rarity"], creature["attack"],
                                        creature["health"])
        raise CardError("CreatureCard not created.")

    def create_spell(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power is None:
            creature = random.choice(self._spells)
            return SpellCard(creature["name"], creature["cost"],
                             creature["rarity"], creature["effect_type"])
        if isinstance(name_or_power, str):
            for creature in self._spells:
                if creature["name"] == name_or_power:
                    return SpellCard(creature["name"], creature["cost"],
                                     creature["rarity"],
                                     creature["effect_type"])
        if isinstance(name_or_power, int):
            for creature in self._spells:
                if creature["cost"] == name_or_power:
                    return SpellCard(creature["name"], creature["cost"],
                                     creature["rarity"],
                                     creature["effect_type"])
        raise CardError("SpellCard not created.")

    def create_artifact(self, name_or_power: str | int | None = None) -> Card:
        if name_or_power is None:
            creature = random.choice(self._artifacts)
            return ArtifactCard(creature["name"], creature["cost"],
                                creature["rarity"],
                                creature["durability"],
                                creature["effect"])

        if isinstance(name_or_power, str):
            for creature in self._artifacts:
                if creature["name"] == name_or_power:
                    return ArtifactCard(creature["name"], creature["cost"],
                                        creature["rarity"],
                                        creature["durability"],
            ...
            for creature in self._artifacts:
                if creature["cost"] == name_or_power:
                    return ArtifactCard(creature["name"], creature["cost"],
                                        ...
                                        creature["durability"],
                                        creature["effect"])
        raise CardError("ArtifactCard not created.")

    def create_themed_deck(self, size: int) -> dict:
        generators = [self.create_creature, self.create_spell,
        ...
        while len(deck) < size:
            random_func = random.choice(generators)
            new_card = random_func()
            if new_card:
    ...
    def get_supported_types(self) -> dict:
        return {"creatures": ["dragon", "goblin"], "spells": ["fireball"],
                "artifacts": ["mana_ring"]}
